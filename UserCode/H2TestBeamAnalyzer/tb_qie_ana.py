...
for ievt in xrange(nevts_to_run):
    ...
    for ichan in xrange(nChs):
        ...
        h_ped_ieta_v_iphi[depth].Fill(ieta,iphi,ped/nevts_to_run)
        h_ped_ieta_v_depth[iphi].Fill(ieta,depth,ped/nevts_to_run)
        h_ped_depth_v_iphi[ieta].Fill(depth,iphi,ped/nevts_to_run)
        ...
        for s in xrange(start,start+nTS):
            ...
            if ntp["qie11"].soi[s] > 0:
                soi = s-start
                if verbose:
                    print "SOI index, soi value:",soi, ntp["qie11"].soi[s]
            if soi != -1 and (s-start) >= soi and (s-start) < soi + sigTS:
                sig = sig + ntp["qie11"].pulse[s]
...
if verbose:
    print "-"*20,"\nPulse data summary: \n","-"*20
for ichan, chan_info in enumerate(shape_info):
    if verbose:
        print "Channel", ichan
